...
class PaymentsView(ctk.CTkFrame):
    ...
    def _build_header(self):
        ...
        self.search_entry = ctk.CTkEntry(
            ...
            width=350, height=42, corner_radius=21, border_width=1,
    ...
    def _build_main_content(self):
        ...
        # --- LEFT: CUSTOMER LIST ---
        table_container = ctk.CTkFrame(content, fg_color="#FFFFFF", corner_radius=16, border_width=1, border_color="#E5E7EB")
        table_container.grid(row=0, column=0, sticky="nsew", padx=(0, 20))
        
        header_row = ctk.CTkFrame(table_container, fg_color="#F9FAFB", corner_radius=8, height=50)
        header_row.pack(fill="x", pady=(15, 5), padx=10)
        ...
        for text, w in cols:
            ctk.CTkLabel(
                ...
                text_color="#6B7280", anchor="w" if text != "Action" else "center"
    ...
    def _show_placeholder(self):
        ...
        ph = ctk.CTkFrame(self.side_panel, fg_color="#FFFFFF", corner_radius=16, border_width=1, border_color="#E5E7EB", height=600)
        ph.pack(fill="both", expand=True)
        ph.pack_propagate(False)
        
        ctk.CTkLabel(ph, text="No Customer Selected", font=ctk.CTkFont(size=16, weight="bold"), text_color="#6B7280").pack(expand=True)
        ctk.CTkLabel(ph, text="Click 📜 Logs or 💳 Pay to view descriptive details.", font=ctk.CTkFont(size=13), text_color="#9CA3AF").pack(expand=True, pady=(0, 200))
    ...
    def _refresh_customers(self):
        ...
        def on_success(result):
            ...
            for c in customers:
                ...
                # Hover effect
                def on_enter(e, r=row): r.configure(fg_color="#F9FAFB")
                def on_leave(e, r=row): r.configure(fg_color="transparent")
                row.bind("<Enter>", on_enter)
                row.bind("<Leave>", on_leave)

                ctk.CTkLabel(row, text=c["shop_name"], width=280, anchor="w", font=ctk.CTkFont(size=14, weight="bold"), text_color="#111827").pack(side="left", padx=15)
                ...
                ctk.CTkLabel(row, text=f"₹{float(c['total_invoiced']):,.2f}", width=100, anchor="w", font=ctk.CTkFont(size=14), text_color="#0EA5E9").pack(side="left", padx=15)
                ...
                ctk.CTkButton(
                    ...
                    fg_color="#F3F4F6", text_color="#374151", hover_color="#E5E7EB", corner_radius=8,
                ...
                ctk.CTkButton(
                    ...
                    fg_color="#0EA5E9", hover_color="#0284C7", corner_radius=8,
                    font=ctk.CTkFont(size=12, weight="bold"),
                    command=lambda cust=c: self._update_panel(cust, "pay")
                ).pack(side="left")

                ctk.CTkFrame(self.scroll_frame, fg_color="#F1F4F9", height=1).pack(fill="x", padx=10)
    ...
    def _build_card(self, title, icon):
        card = ctk.CTkFrame(self.side_panel, fg_color="#FFFFFF", corner_radius=16, border_width=1, border_color="#E5E7EB")
        card.pack(fill="x", pady=(0, 15))
        
        header = ctk.CTkFrame(card, fg_color="transparent")
        header.pack(fill="x", padx=20, pady=(15, 10))
        ctk.CTkLabel(header, text=f"{icon} {title}", font=ctk.CTkFont(size=15, weight="bold"), text_color="#111827").pack(side="left")
        return card
    ...
    def _update_panel(self, customer, mode):
        ...
        ctk.CTkLabel(pb, text=customer["shop_name"], font=ctk.CTkFont(size=14, weight="bold"), text_color="#0EA5E9").pack(pady=(10, 2))
    ...
    def _render_invoice_history(self, customer, card):
        ...
        h = ctk.CTkFrame(container, fg_color="#F9FAFB", height=30)
        h.pack(fill="x", pady=5)
        for t, w in [("Inv No", 80), ("Date", 80), ("Status", 80)]:
            ctk.CTkLabel(h, text=t, width=w, font=ctk.CTkFont(size=11, weight="bold"), text_color="#6B7280").pack(side="left", padx=5)
    ...
    def _render_payment_logs(self, customer, card):
        ...
        for p in payments[:10]:
            ...
            ctk.CTkFrame(container, fg_color="#F1F4F9", height=1).pack(fill="x")
    ...
    def _render_payment_form(self, customer, card):
        ...
        ctk.CTkOptionMenu(container, values=["Cash", "UPI", "Bank"], variable=mode_var, height=35, fg_color="#0EA5E9").pack(fill="x", pady=10)
        
        dt_entry = ctk.CTkEntry(
            container, height=35, fg_color="#F9FAFB", border_color="#E5E7EB",
            text_color="#111827"
        )
